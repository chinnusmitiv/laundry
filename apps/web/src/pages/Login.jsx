...
// Passwordless email/phone + OTP login — mirrors the customer mobile app.
    ...
    <div className="app-page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="panel" style={{ width: '100%', maxWidth: 440 }}>
        ...
        <p className="cl-muted" style={{ textAlign: 'center', marginBottom: 24 }}>
          {step === 'identify' ? 'No password needed — we send a one-time code.' : <>Sent to <b>{sent?.sent_to}</b></>}
        </p>

        {step === 'identify' ? <>
          <label className="cl-label">Email or phone number</label>
          <input className="cl-field" style={{ width: '100%', marginBottom: 14 }} placeholder="you@email.com  ·  9123 4567"
          ...
          <p className="cl-muted" style={{ fontSize: 13, textAlign: 'center', marginTop: 14 }}>
            We'll {isPhone ? 'text' : 'email'} you a 6-digit code.
          </p>
        </> : <>
          {sent?.is_new && <>
          ...
          {sent?.dev_code && (
            <div style={{ background: 'var(--lime-pale)', border: '1.5px dashed var(--lime-d)', color: 'var(--navy)', fontSize: 13, padding: '10px 12px', borderRadius: 10, marginBottom: 14 }}>
              🔒 <b>Demo mode</b> — a real app would {sent.channel === 'phone' ? 'SMS' : 'email'} this. Code: <b>{sent.dev_code}</b>
            </div>
          )}
          {err && <ErrBox>{err}</ErrBox>}
        ...
        {step === 'identify' && (
          <button onClick={() => setIdentifier('alex@example.com')} style={{ display: 'block', margin: '16px auto 0', color: 'var(--gray)', fontSize: 13, fontWeight: 600 }}>
            Use demo account →
          </button>
        )}
      </div>
